...
data_ip="https://gitea.com/xdarnix/juall/raw/branch/main/ip"
checking_sc() {
    ...
    if [[ ${splvm} != "" ]]; then
        for vmuser in ${vm[@]}; do
            ...
            if [[ ${vmhas} -gt $tess ]]; then
                ...
                gb=$(convert ${byt})
                mb=$(echo "${gb} * 1024" | bc) # Conversión de GB a MB
                lim=$(cat /etc/vmess/${vmuser})
                lim2=$(convert ${lim})
                lim2_mb=$(echo "${lim2} * 1024" | bc) # Conversión de límite de GB a MB
                echo -e "$COLOR1${NC} USERNAME : \033[0;33m$vmuser"
                echo -e "$COLOR1${NC} IP LOGIN : \033[0;33m$vmhas"
                echo -e "$COLOR1${NC} USAGE : \033[0;33m$gb GB (\033[0;33m$mb MB)"
                echo -e "$COLOR1${NC} LIMIT : \033[0;33m$lim2 GB (\033[0;33m$lim2_mb MB)"
                echo -e ""
            fi
        done
    fi
